...
def extract_c_structure(lines, filename):
    """
    ...
    functions = extract_c_functions(lines)
    structs = extract_c_structs(lines)

    # Calculate statistics
    function_count = len(functions)
    struct_count = len(structs)

    # Format parameters for each function to match the expected format
    for function in functions:
        # Convert parameters from list of dicts to list of names
        if "parameters" in function:
            param_names = [param.get("name", "") for param in function["parameters"]]
            function["parameters"] = param_names
...
def extract_code_structure(code, filename, ast_info=None):
    """
    Extracts the code structure from the source code.
    If ast_info is provided, it will be used instead of generating a new one.

    Args:
        code (str): The source code to analyze
        filename (str): Name of the source file
        ast_info (dict, optional): Pre-extracted AST information

    Returns:
        dict: A structured representation of the code
    """
    # If AST info is already provided, use it
    if ast_info:
        return {
            "file_path": filename,
            "ast_info": ast_info
    ...
    # Otherwise, generate a minimal structure for Java code
    language = get_language_from_filename(filename)

    if language == 'java':
        # Split the code into lines for analysis
        lines = code.split('\n')
        return extract_java_structure(lines, filename)
    else:
        # Default structure for unknown languages
        # Create a minimal AST info structure
        lines = code.split('\n')
        ast_info = {
            "language": language,
            "content_summary": f"{len(lines)} lines of code",
...
def generate_prompt(code, filename, ast_info=None):
    """
    Generates a markdown prompt template for analyzing code vulnerabilities.
    Includes code structure extraction to assist with vulnerability detection and localization.
    If ast_info is provided, it will be used instead of generating a new one.
    ...
    # Extract code structure using the provided AST info if available
    structure = extract_code_structure(code, filename, ast_info)

    # Extract the AST info for the prompt
    ast_info = structure.get("ast_info", {})

    # Convert to JSON string with indentation for readability
    ast_info_json = json.dumps(ast_info, indent=2)
...
def generate_analysis_prompt(response):
    """
    Generates a prompt for analyzing an LLM response.
...
def extract_fields(entry):
    ...
    Returns:
        tuple: A tuple containing the code, filename, identification fields, and AST info.
    """
    code = entry.get('code', '')
    ...
    ast_info = entry.get('ast_info', None)
    return code, filename, entry_id, sub_id, code_id, ast_info
...
def interact_with_llm(entry, custom_prompt, model_name, config):
    ...
    Preserves the original response and adds a new field for the relevance analysis.
    ...
    try:
        ...
        # Preserve all original fields and add the relevance analysis
        new_entry = {
            'id': entry_id,
            'sub_id': sub_id,
            'code_id': code_id,
            'prompt_eval_count': entry.get('prompt_eval_count', 0),
            'prompt_eval_duration': entry.get('prompt_eval_duration', 0),
            'eval_count': entry.get('eval_count', 0),
            'eval_duration': entry.get('eval_duration', 0),
            'total_duration': entry.get('total_duration', 0),
            'load_duration': entry.get('load_duration', 0),
            'response': entry.get('response', ''),  # Preserve original response
            # Add new assessment field
            'relevance_analysis': response['message']['content']
        }
        return new_entry
    except Exception as e:
