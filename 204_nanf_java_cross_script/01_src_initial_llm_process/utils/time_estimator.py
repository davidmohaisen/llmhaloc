...
class TimeEstimator:
    ...
    def __init__(self, model_name, total_entries, log_dir, resume_index=0):
        ...
        # Recent times tracking (for weighted average)
        self.recent_times = []
        self.max_recent_times = 10  # Number of recent entries to track for weighted average
        ...
        if os.path.exists(times_file) and os.path.getsize(times_file) > 0:
            try:
                with open(times_file, 'r') as file:
                    ...
                    # Initialize recent times with the last few entries if available
                    if len(self.processing_times) > 0:
                        self.recent_times = self.processing_times[-min(self.max_recent_times, len(self.processing_times)):]
    ...
    def get_estimates(self):
        ...
        # Use weighted average for estimation if available, otherwise use regular average
        time_per_entry = weighted_avg if weighted_avg > 0 else avg_time
        estimated_remaining_time = time_per_entry * self.remaining_entries
        ...
        entries_per_minute = 60 / time_per_entry if time_per_entry > 0 else 0
    ...
    def _calculate_weighted_average(self):
        """
        Calculate a weighted average of processing times, giving more weight to recent entries.

        Returns:
            float: Weighted average time per entry
        """
        if not self.recent_times:
            return 0

        # Assign weights (more recent entries have higher weights)
        weights = [i+1 for i in range(len(self.recent_times))]
        total_weight = sum(weights)

        # Calculate weighted average
        weighted_sum = sum(t * w for t, w in zip(self.recent_times, weights))
    ...
    def _log_initial_estimate(self):
        ...
        weighted_avg = estimates['weighted_avg_time']
        ...
        if weighted_avg > 0:
            logger.info(f"Weighted average (recent entries): {weighted_avg:.2f} seconds per entry")
