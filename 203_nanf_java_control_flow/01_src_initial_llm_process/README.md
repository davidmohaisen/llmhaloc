# Initial LLM Processing for Vulnerability Function Localization with Control Flow Analysis


## Purpose

This directory contains scripts for the initial processing of code samples with Large Language Models (LLMs) to identify vulnerable functions. The scripts process a dataset of code samples, send them to various LLMs, and collect the responses for further analysis. This version enhances the analysis by incorporating control flow information to help LLMs better understand the code structure and identify vulnerabilities more accurately.
...
│   ├── global_time_tracker.py  # Global time tracking across models
...
- **GlobalTimeTracker**: Tracks processing time across all models
- **TimeEstimator**: Tracks processing time for individual models
...
- `utils/global_time_tracker.py` - Tracks and estimates processing time across all models
...
4. **Model Processing**: For each model in the machine's model list:
   ...
   - Extract control flow information to enhance the prompt
   - Process entries with immediate retry mechanism for error handling
...
The system enhances the prompts with control flow information:

1. **Control Flow Extraction**: Extract control flow information from the code
2. **Enhanced Prompts**: Include control flow information in the prompts sent to LLMs
3. **Improved Understanding**: Help LLMs better understand the code structure and relationships
4. **Better Vulnerability Detection**: Enable more accurate identification of vulnerable functions
...
- **Control Flow Analysis**: Enhances prompts with control flow information for better vulnerability detection
- **Immediate Retry Mechanism**: Automatically retries failed entries up to a configurable maximum
- **Error Tracking**: Creates error files for failed entries with detailed error information
- **Modular Code Structure**: Breaks down complex functions into smaller, more manageable pieces
...
# Run with verbose mode (displays detailed prompt information)
python main.py --verbose

# Run with custom maximum retry attempts
python main.py --max-retries 5

# Reset resume points for all models
python main.py --reset

# Reset resume point for a specific model
python main.py --reset-model llama3_70b_instruct

# Clear failed entries for all models
python main.py --clear-failed

# Clear failed entries for a specific model
python main.py --clear-failed-model llama3_70b_instruct
```
...
- Failed entries are automatically retried up to the configured maximum
- Error files are created for failed entries with detailed error information
- The system stops processing if an entry fails after the maximum number of retries
- Control flow information is included in the prompts to enhance vulnerability detection

## Recent Improvements

- **Modular Code Structure**: Refactored complex functions into smaller, more manageable pieces
- **Immediate Retry Mechanism**: Added automatic retry for failed entries
- **Error Tracking**: Added detailed error tracking with error files
- **Robust Error Handling**: Improved error handling with proper cleanup
- **Logger Initialization**: Fixed logger initialization to handle missing attributes
- **Cognitive Complexity Reduction**: Reduced cognitive complexity of complex functions
- **Control Flow Analysis**: Added control flow information to enhance prompts
- **Command Line Options**: Added more command line options for better control
- **Documentation**: Updated documentation with detailed information about the system
